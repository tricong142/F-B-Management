// =============================================================================
//  CashierTables — sơ đồ bàn cho thu ngân
//  Tính năng (giảng viên review):
//   - Right-click hoặc long-press bàn → mở TableContextMenu
//   - Chuyển bàn / dọn bàn / tạm khoá-bật bàn
//   - Realtime: bàn cập nhật ngay khi waiter gửi đơn / khoá bàn
// =============================================================================
...
import { useTableMenu } from '../components/TableContextMenu';
...
export default function CashierTables() {
  ...
  // Realtime: nhiều event đều có thể đổi trạng thái bàn
  useSocket({
    'tables:changed':  load,
    'orders:changed':  load,
    'order:created':   load,
  ...
  // Map theo code để TableContextMenu tra cứu nhanh
  const tablesByCode = useMemo(() => {
    const m = {};
    tables.forEach(t => { if (t.code) m[t.code] = t; });
    return m;
  }, [tables]);

  const menu = useTableMenu({ tablesById: tablesByCode, onChanged: load });
  ...
  const onClickTable = (t) => {
    ...
    navigate('/cashier/detail/' + encodeURIComponent(t.code));
  };
  ...
  return (
    <div className="max-w-6xl mx-auto px-4 py-4">
      {/* Heading */}
      <div className="mb-4">
        <h1 className="text-xl font-bold text-on-surface">Sơ đồ bàn</h1>
        <p className="text-sm text-muted">
          Thu ngân — chọn bàn để xem &amp; thanh toán.
          <span className="ml-1 text-xs">(Chuột phải / nhấn giữ để mở menu chuyển/dọn/tắt bàn)</span>
        </p>
      </div>
      ...
      <div className="flex flex-wrap gap-2 mb-3 items-center">
        ...
        <span className="px-3 py-1 rounded-full text-xs font-semibold pill-off">Tắt</span>
      </div>
      ...
      {loading && tables.length === 0 ? (
        ...
        >
          {filtered.map(t => (
            <TableCard key={t.id} t={t} onClick={() => onClickTable(t)} />
          ))}
        </div>
      )}

      {/* Floating elements (menu, move dialog) */}
      <menu.Element />
    </div>
  );
}
...
// ─── Sub: 1 thẻ bàn ───────────────────────────────────────────────────────
function TableCard({ t, onClick }) {
  const status = t.is_active === false ? 'inactive' : (t.status || 'empty');
  const inactive = status === 'inactive';
  return (
    <button
      data-table-id={t.code}
      onClick={onClick}
      className={
        'card p-4 text-left transition relative overflow-hidden ' +
        (inactive
    ...
    >
        ...
        <>
          <div className="flex items-center justify-between mt-3 pt-3 border-t border-border-soft">
            <div className="flex items-center gap-1 text-xs text-muted">
              ...
              <span>{t.mins || 0}ph</span>
            </div>
            <div className="font-bold text-primary">{fmt(t.spent || 0)}</div>
          </div>
          {status === 'pay' && (
            <div className="mt-2 text-xs text-red-700 bg-red-50 px-2 py-1 rounded-lg">
              🔔 Đơn mới: {fmt(t.spent || 0)}
            </div>
          )}
        </>
      )}
      ...
      {status === 'empty' && (
        <div className="mt-3 text-xs text-muted">Trống</div>
      )}
    </button>
  );
}
